...
<xsl:template match="JarAnalyzer">
    ...
    <xsl:for-each select="./Jars/Jar">
        <xsl:if test="count(error) = 0">
            <tr>
                ...
                <td align="right"><xsl:value-of select="Summary/Statistics/Level"/></td>
    ...
    <xsl:for-each select="./Jars/Jar">
        <xsl:if test="count(error) = 0">
            ...
            <table width="100%" class="gray"><tr>
				<td><a href="#EXlevel">Level</a>: <xsl:value-of select="Summary/Statistics/Level"/></td>
 ...
 <hr size="2" />    
    ...
    <h3><a name="EXclassnumber">Number of Classes</a></h3>
       ...
       <h3><a name="EXlevel">Level</a></h3>
        <p>The Level represents where in the hierarchy a jar file lives. Level 1 jars are at the bottom. Level 2 depend on at least one Level 1. Level 3 depend on at least one Level 2. The Level of the jar, used in conjunction with Instability, gives an indication of the jar's resilience to change.</p> 
